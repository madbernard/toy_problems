...
function fight(robot1, robot2, tactics) {
  var winner = false;
  var bySpeed = robot1.speed < robot2.speed ? [robot2, robot1] : [robot1, robot2];
  var fast = bySpeed[0];
  var slow = bySpeed[1];
  var rounds = Math.max(robot1.tactics.length, robot2.tactics.length);

  for (var i = 0; i < rounds; i++) {
    slow.health = slow.health - (fast.tactics[i] ? tactics[fast.tactics[i]] : 0);
    winner = isKO(fast, slow);
    if (winner) break;

    fast.health = fast.health - (slow.tactics[i] ? tactics[slow.tactics[i]] : 0);
    winner = isKO(fast, slow);
    if (winner) break;
  }

  winner = fast.health === slow.health ? false : (fast.health > slow.health ? fast : slow);

  return winner ? `${winner.name} has won the fight.` : 'The fight was a draw.';
}

function isKO(one, two) {
  if (Math.min(one.health, two.health) < 1) {
    return one.health ? one : two;
  }
  return false;
};
