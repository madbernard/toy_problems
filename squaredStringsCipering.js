/*
  https://www.codewars.com/kata/coding-with-squared-strings/javascript
...
*/
function code(s) {
  var squareSize = Math.ceil(Math.sqrt(s.length));
  while (s.length < (squareSize * squareSize)) {
    s = s + String.fromCharCode(11);
  }

  var codeArr = [];
  for (let i = s.length - 1; i > -1; i--) {
    var idx = i % squareSize;
    codeArr[idx] = codeArr[idx] ? codeArr[idx] + s[i] : '' + s[i];
  }
  return codeArr.join('\n');;
}
  /*
...
function decode(s) {
  s = s.replace(/\n/g, '');
  var squareSize = Math.sqrt(s.length);
  var codeArr = [];
  for(let i = 0; i < s.length; i++){
    var idx = squareSize - (i % squareSize);
    codeArr[idx] = codeArr[idx] ? codeArr[idx] + s[i] : '' + s[i];
  }
  var result = codeArr.join('');
  return result.substring(0, result.indexOf(String.fromCharCode(11)) );
}
